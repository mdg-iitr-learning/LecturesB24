...
class Metal(Material):
    ...
    def scatter(self, ray_in, reco):
        ...
        scat = Ray(reco.p, x - 2*x.dot(reco.norm)*reco.norm + self.fuzz*rando())
...
class Dielectric(Material):
    ...
    def scatter(self, ray_in, reco):
        ...
        x = u(ray_in.direction)
        refl = x - 2*x.dot(reco.norm)*reco.norm
        
        if x.dot(reco.norm) > 0:
            out = -reco.norm
            n = self.ri
            cos = self.ri * x.dot(reco.norm)
        else:
            out = reco.norm
            n = 1/self.ri
            cos = -x.dot(reco.norm)
        
        r,refr = refract(x, out, n)
        if r:
            prob = schlick(cos, self.ri)
        else:
            scat = Ray(reco.p, refl)
            prob = 1

        if random.random() < prob:
            scat = Ray(reco.p, refl)
        else:
            scat = Ray(reco.p, refr)
        return (True, scat, atte)
...
def refract(v, n, ni):
    ...
    dis = 1 - ni*ni*(1-dt*dt)
    if dis > 0:
        return (True, ni*(x - n*dt) - n*(dis**0.5))
    return (False, None)
...
with open('9.ppm', 'w') as f:
    print('P3\n{0} {1}\n255\n'.format(nx, ny), file = f)
    camera = Camera()
    world = HitableList([
        Sphere(v([0.0, 0, -1]), 0.5, Lambertian(v([0.8, 0.3, 0.3]))),
        Sphere(v([0, -100.5, -1]), 100, Lambertian(v([0.8, 0.8, 0]))),
        Sphere(v([1.0, 0, -1]), 0.5, Metal(v([0.8, 0.6, 0.2]), 0.5)),
        Sphere(v([-1.0, 0, -1]), 0.5, Dielectric(v([0.8, 0.8, 0.8]), 1.5))
    ])
