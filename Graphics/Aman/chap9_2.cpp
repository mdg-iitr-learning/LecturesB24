...
bool refract(const vec3& v, const vec3& n, float ni_over_nt,vec3& refracated){
    vec3 uv= unit_vector(v);
    float dt = dot(uv,n);
    float discriminant = 1.0 -(ni_over_nt*ni_over_nt)*(1-dt*dt);
    if(discriminant >0){
        refracated = ni_over_nt*(uv - n*dt) - n*sqrt(discriminant);
        return true;
    }
    else 
        return false;
...
int main(){
    ...
    int ns=100;
    fout<<"P3\n"<<nx<<" "<<ny<<"\n255\n";
    static lambertain diffuse_grey(vec3(0.1,0.2,0.5));
